...
function updateAppInterface() {
    ...
    displayTransactions.forEach(t => {
        ...
        // Renderiza os dados primários da linha
        row.innerHTML = `
            <td>
                <strong>${safeDesc}</strong> ${t.status === 'pending' ? '<span style="font-size:0.65rem; background:#c5a059; color:#fff; padding:2px 6px; border-radius:4px; margin-left:5px;">⏳ PREVISTO</span>' : ''}<br>
        ...
        // Busca a célula de ação que acabamos de criar
...
window.toggleFutureFields = function() {
    const isPending = document.querySelector('input[name="status"]:checked').value === 'pending';
    document.getElementById('future-fields').style.display = isPending ? 'flex' : 'none';
};
        ...
        if (diffDays <= 15) {
            ...
            alertDiv.innerHTML = `
                ...
                <div class="alert-content">
                    ...
                    <span>${statusText} — ${formatCurrency(t.amount)}</span>
                </div>
            `;
